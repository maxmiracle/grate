...
namespace Grate.ColorDispersion
{
    public class ColorGroup : ColorElement, IEnumerable<ColorElement>
    {
        ...
        {
            AddElementOrGroupAsChild(cw1);
            AddElementOrGroupAsChild(cw2);
            RecalculateMetrics();
            level = EstimateLevel(ColorWeight.Delta(cw1, cw2));
        }

        private void AddElementOrGroupAsChild(ColorWeight cw)
        {
        ...
        public string Text
        {
            get
            {
                return String.Format("{0} L:{1}", base.Text, Level);
            }
        }
